...
def make_tokens(string):
    ...
    # Splits on word, digit and parenthesis, but not words and digits withn parenthesis
...
#Using the even-odd structure of mathematical expressions to check syntax
...
# Returns the index of the operator with the highest precedence
def get_precedence_index(expression):
    ...
    for i in operator_precedence:
        ...
        if index:
            return index
    abort(501, description="Internal error: get_precedence_index() cant find next operator.")
...
# Takes a a singel symbol and returns a Term
def evaluate_symbol(symbol):
    if isinstance(symbol, Symbols.Term):
        return symbol.value
    if isinstance(symbol, Symbols.Expression):
        # If the symbol is an expression, evaluate the expression before evaluation of the symbol
...
def calc_json_expression(json_expression):
    ...
    check_syntax(symbols_sequence.get())
